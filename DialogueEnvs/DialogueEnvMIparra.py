...
import numpy as np
from Users.UserMI_parra import UserMI
import torch
class DialogueEnvMI(gym.Env):
    ...
    def __init__(self,n_parra=5, render_mode=None):
        ...
        high = np.array(
            [
                1,
            ]*( self.n_agent_da+ self.n_user_da+768),
            dtype=np.int32,
        )
        low = np.array(
            [
    ...
    def _get_obs(self):
        ...
        master_ob = np.array([[self.user.context[i]/3,self.user.rapport[i]/3,self.user.perspective[i]/3] for i in range(self.n_parra)])
        print(master_ob)
        ob = np.concatenate([user_da,agent_da,self.user.encoded_text],axis=1)
        return master_ob,ob
