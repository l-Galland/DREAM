...
class SACAgent:
    ...
    def __init__(self, observation_space, action_space,len_replay_buffer=1000000,replay_mini_batch_size=1000000,learning_rate=10**-4,use_maml=False):
        ...
        self.actor_maml = l2l.algorithms.MAML(self.actor_local, lr=self.LEARNING_RATE, first_order=False)
        self.actor_maml_opt = torch.optim.Adam(self.actor_maml.parameters(), lr=4*self.LEARNING_RATE/10)
        self.actor_learner = self.actor_maml.clone()
        self.critic_maml = l2l.algorithms.MAML(self.critic_local, lr=self.LEARNING_RATE, first_order=False)
        self.critic_maml_opt = torch.optim.Adam(self.critic_maml.parameters(), lr=4*self.LEARNING_RATE/10)
        self.critic_learner = self.critic_maml.clone()
        self.critic2_maml = l2l.algorithms.MAML(self.critic_local, lr=self.LEARNING_RATE, first_order=False)
        self.critic2_maml_opt = torch.optim.Adam(self.critic2_maml.parameters(), lr=4*self.LEARNING_RATE/10)
    ...
    def get_next_action(self, state, evaluation_episode=False):
        if evaluation_episode:
            discrete_action = self.get_action_deterministically(state,self.use_maml)
        else:
            discrete_action = self.get_action_nondeterministically(state,self.use_maml)
        print(discrete_action)
        return discrete_action

    def get_action_nondeterministically(self, state,use_maml):
        action_probabilities = self.get_action_probabilities(state,use_maml)
    ...
    def get_action_deterministically(self, state,use_maml):
        action_probabilities = self.get_action_probabilities(state,use_maml)
    ...
    def train_networks(self, transition):
        ...
        if self.replay_buffer.get_size() >self.REPLAY_BUFFER_BATCH_SIZE:
            ...
            critic_loss, critic2_loss = \
                self.critic_loss(states_tensor, actions_tensor, rewards_tensor, next_states_tensor, done_tensor,self.use_maml)
            ...
            actor_loss, log_action_probabilities = self.actor_loss(states_tensor,self.use_maml)
    ...
    def critic_loss(self, states_tensor, actions_tensor, rewards_tensor, next_states_tensor, done_tensor,use_maml):
        if use_maml:
            with torch.no_grad():
                action_probabilities, log_action_probabilities = self.get_action_info(next_states_tensor,use_maml)
            ...
            soft_q_values = self.critic_learner(states_tensor).gather(1, actions_tensor.unsqueeze(-1)).squeeze(-1)
            soft_q_values2 = self.critic2_learner(states_tensor).gather(1, actions_tensor.unsqueeze(-1)).squeeze(-1)
    ...
    def actor_loss(self, states_tensor,use_maml):
        if use_maml:
            action_probabilities, log_action_probabilities = self.get_action_info(states_tensor,use_maml)
            with torch.no_grad():
                q_values_local = self.critic_learner(states_tensor)
                q_values_local2 = self.critic2_learner(states_tensor)
    ...
    def get_action_info(self, states_tensor,use_maml):
        if use_maml:
            action_probabilities = self.actor_learner.forward(states_tensor)
        else:
    ...
    def get_action_probabilities(self, state,use_maml):
        
        state_tensor = torch.tensor(state, dtype=torch.float32).unsqueeze(0)
        if use_maml:
            action_probabilities = self.actor_learner.forward(state_tensor)
        else:
